...
function Login() {
    ...
    function handleSubmit(e) {
        ...
        if (!Object.keys(errs).length) {
            // Place login API logic here
            alert(
                `Logging in with ${isEmail(identity) ? 'email' : 'phone'}: ${identity}`
            );
        }
    }
            ...
            <div className="w-full max-w-lg mx-auto bg-white rounded-2xl shadow-xl border border-[#ede8de] flex flex-col md:flex-row overflow-hidden">
                ...
                <div className="hidden md:flex flex-col justify-between items-center bg-gradient-to-b from-[#d9b382]/90 to-[#b7c6a0]/80 w-2/5 px-6 py-10">
                    <div className="w-full text-left">
                        ...
                        <h2 className="text-2xl font-bold text-[#3e2f26] mb-3">Welcome Back!</h2>
                        <p className="text-[#514636] text-sm">
                            Sign in to track your <span className="font-semibold">orders</span>, manage your <span className="font-semibold">wishlist</span>, and enjoy exclusive <span className="font-semibold">farmer offers</span>.
                        </p>
                    </div>
                    <img
                ...
                <div className="flex-1 flex flex-col justify-center px-6 py-10">
                    <form className="w-full max-w-xs mx-auto" onSubmit={handleSubmit} autoComplete="off">
                        <div className="flex flex-col gap-1 mb-7">
                            <label className="text-sm font-medium text-[#3e2f26]">Email or Phone Number</label>
                            <input
                                type="text"
                                ...
                                autoFocus
                                value={identity}
                                onChange={e => { setIdentity(e.target.value); setSubmitted(false); setErrors({ ...errors, identity: undefined }); }}
                            />
                            {errors.identity && submitted && (
                                <span className="text-xs text-red-500 mt-1">{errors.identity}</span>
                            )}
                        </div>
                        <div className="flex flex-col gap-1 mb-7">
                            ...
                            <input
                                ...
                                value={password}
                                onChange={e => { setPassword(e.target.value); setSubmitted(false); setErrors({ ...errors, password: undefined }); }}
                            />
                        ...
                        <div className="text-center text-[#2176ff] text-sm mt-4 cursor-pointer hover:underline" onClick={()=>navigate('/signup')}>
